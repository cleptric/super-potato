                ...
                <div class="p-3 bg-white shadow rounded-lg overflow-hidden">
                    ...
                    <dd class="mt-1 text-3xl font-semibold text-gray-900">
                        ...
                        <template v-else>
                            <span class="relative z-0 inline-flex shadow-sm rounded-md">
                                <button
                                    @click="triggerRunwayClosed('16C/34C')"
                                    ...
                                    :class="{ 'border-red-300 bg-red-200 text-red-800 hover:bg-red-100 focus:ring-red-500 focus:border-red-500': lowg.closed_runways.includes('16C/34C') }"
                                >
                                    <i v-if="lowg.closed_runways.includes('16C/34C')" class="far fa-times mr-1 text-red-800"></i> 16C / 34C
                                </button>
                            </span>
                        </template>
                    </dd>
    ...
    setup () {
        ...
        return {
            ...
            missedApporachDisabled: computed(() => {
                ...
                return timestamp < store.getters.lowg.missed_approach_timeout
            }),
            closedRunwaysDisabled: computed(() => {
                let timestamp = Math.round((new Date()).getTime() / 1000)
                return timestamp < store.getters.lowg.closed_runways_timeout
            }),
        }
    },
    data() {
        ...
        startMissedApproachTimer() {
            if (this.missedApporachInterval === null && this.missedApporachDisabled) {
                this.missedApporachInterval = setInterval(() => {
                    let timer = this.$store.getters.lowg.missed_approach_timeout - Math.round((new Date()).getTime() / 1000)
                    if (timer < 0) {
                        timer = 0
                    }
        ...
        startClosedRunwaysTimer() {
            if (this.closedRunwaysInterval === null && this.closedRunwaysDisabled) {
                this.closedRunwaysInterval = setInterval(() => {
                    let timer = this.$store.getters.lowg.closed_runways_timeout - Math.round((new Date()).getTime() / 1000)
                    if (timer < 0) {
                        timer = 0
                    }
